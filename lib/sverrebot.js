...
SverreBot.prototype._welcomeMessage = function () {
    this.channels.forEach(function (channel) {
        if(channel.is_member) {
            this.postMessageToChannel(channel.name, 'Hei alle sammen! Nå kommer det vitser på rekke og rad! ' +
...
SverreBot.prototype._replyWithRandomJoke = function (originalMessage) {
    ...
    self.db.get('SELECT id, joke FROM jokes ORDER BY used ASC, RANDOM() LIMIT 1', function (err, record) {
        ...
        while (message.includes('Chuck')) message = message.replace('Chuck', 'Sverre');
        while (message.includes('Norris'))message = message.replace('Norris', 'Hurum');
        while (message.includes('Sverre Hurum\'')) message = message.replace('Sverre Hurum\'', 'Sverre Hurums');
